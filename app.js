function showTool(id){document.querySelectorAll('.tool-panel').forEach(p=>p.classList.remove('active'));const p=document.getElementById('panel-'+id);if(p)p.classList.add('active');document.getElementById('mobileTools').classList.add('hidden');window.scrollTo({top:0,behavior:'smooth'})}
...
function animateProgress(id,ms,done){const bar=document.getElementById(id);let p=0;bar.style.width='0%';const step=100/(ms/40);const t=setInterval(()=>{p=Math.min(100,p+step+Math.random()*3);bar.style.width=p+'%';if(p>=100){clearInterval(t);if(done)done()}},40)}
...
function estimateCrack(){const pwd=document.getElementById('crackInput').value;const out=document.getElementById('crackResult');if(!pwd){out.innerHTML='';return}let charset=0;if(/[a-z]/.test(pwd))charset+=26;if(/[A-Z]/.test(pwd))charset+=26;if(/[0-9]/.test(pwd))charset+=10;if(/[^a-zA-Z0-9]/.test(pwd))charset+=32;if(charset<2)charset=26;const entropy=pwd.length*Math.log2(charset);const seconds=Math.pow(2,entropy)/1e10;let human;if(seconds<1)human='Menos de 1 segundo';else if(seconds<60)human=seconds.toFixed(1)+' segundos';else if(seconds<3600)human=(seconds/60).toFixed(1)+' minutos';else if(seconds<86400)human=(seconds/3600).toFixed(1)+' horas';else if(seconds<31536000)human=(seconds/86400).toFixed(1)+' días';else if(seconds<31536000*100)human=(seconds/31536000).toFixed(1)+' años';else if(seconds<31536000*1e6)human=(seconds/31536000/1000).toFixed(0)+' mil años';else human='Prácticamente inviable';out.innerHTML=`<p>Longitud ${pwd.length} · charset ≈ ${charset} · ≈ ${entropy.toFixed(0)} bits</p><p class="text-white font-medium">Estimación offline: ${human}</p>`}
...
const brief=(d,name,data,year)=>{
  const t=(d||'').trim();
  if(t.length>40){let s=t;if(s.length>280)s=s.slice(0,277)+'…';return s;}
  const types=(data||'').split(/[;|,]/).map(x=>x.trim()).filter(Boolean);
  const top=types.slice(0,4).join(', ')||'datos de cuenta';
  const y=year?` hacia ${year}`:'';
  return `Incidente asociado a ${name||'un servicio'}${y}. Entre los datos expuestos suelen figurar: ${top}. Conviene tratar la contraseña de ese servicio (y cualquier reutilización) como comprometida.`;
};
try{
  bar.style.width='40%';status.textContent='Obteniendo detalle de filtraciones…';
  ...
  if(res.status===429){
    ...
    document.getElementById('breachSummary').innerHTML=`<div class="p-5 rounded-2xl border border-amber-500/30 bg-amber-500/10"><p class="text-amber-200 font-medium">Demasiadas consultas desde esta red</p><p class="text-steel text-sm mt-2">El índice público limita las peticiones por hora. Espera un rato e inténtalo de nuevo.</p></div>`;
  ...
  if(!details.length){
    ...
    document.getElementById('breachList').innerHTML=details.map(b=>{
      ...
      const chips=types.map(t=>`<span class="text-[11px] px-2 py-0.5 rounded-md bg-red-500/15 text-red-300 border border-red-500/20">${esc(t)}</span>`).join('')||'<span class="text-xs text-steel">Tipos de dato no especificados</span>';
      const exp=esc(brief(b.details,b.breach,b.xposed_data,b.xposed_date));
      const records=b.xposed_records?`<span class="text-xs text-steel">· ~${Number(b.xposed_records).toLocaleString('es')} registros en el incidente</span>`:'';
      return `<div class="p-4 rounded-xl bg-void border border-white/10 space-y-2"><div class="flex flex-wrap items-baseline justify-between gap-2"><p class="text-white font-medium">${name}</p><span class="text-xs text-steel">${year}${domain?' · '+domain:''}</span></div><div class="flex flex-wrap gap-1.5">${chips}</div><p class="text-xs text-steel leading-relaxed">${exp}</p>${records?`<p>${records}</p>`:''}</div>`;
    }).join('');
  }
  results.classList.remove('hidden');
}catch(e){
  bar.style.width='100%';status.textContent='No se pudo completar la consulta';
  document.getElementById('breachSummary').innerHTML=`<div class="p-5 rounded-2xl border border-amber-500/30 bg-amber-500/10"><p class="text-amber-200 font-medium">Consulta no disponible ahora</p><p class="text-steel text-sm mt-2">Puede deberse a red, CORS o límite temporal del índice. El plan de respuesta de abajo sigue siendo válido.</p></div>`;
  document.getElementById('breachList').innerHTML='';
  results.classList.remove('hidden');
}
btn.disabled=false;
...
function updateCompLabel(){const v=+document.getElementById('compQual').value;let label='Muy baja';if(v>=0.9)label='Nítida';else if(v>=0.75)label='Buena calidad';else if(v>=0.55)label='Calidad media';else if(v>=0.35)label='Calidad baja';document.getElementById('compQualLabel').textContent=label+' ('+v.toFixed(2)+')'}
updateCompLabel();
